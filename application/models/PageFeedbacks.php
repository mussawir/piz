...
class Application_Model_PageFeedbacks extends Zend_Db_Table
{ 
...
public function getCommentsByPage($id,$feed_id) 
{
$select = $this->select();
$select->from($this)->where("page_id = ?", $id)->where("feed_id = ?", $feed_id)->order('comment_date DESC');
$result = $this->fetchAll($select);
return $result;
}
